...
ConVar asw_fire_extinguisher_infinite("asw_fire_extinguisher_infinite", "0", FCVAR_CHEAT, "Infinite ammo for fire extinguisher.");
ConVar asw_fire_extinguisher_freeze_amount("asw_fire_extinguisher_freeze_amount", "0.4", FCVAR_CHEAT, "Adjusts the freeze time for the fire extinguisher.");
...
void CASW_Weapon_FireExtinguisher::PrimaryAttack( void )
{
	...
	if (pMarine)		// firing from a marine
	{
		...
		if ( pMarine->GetWaterLevel() != 3 )
		{
			...
			newVel.z -= 40.0f;
			CASW_Extinguisher_Projectile *projectile = CASW_Extinguisher_Projectile::Extinguisher_Projectile_Create( vecSrc, QAngle(0,0,0),
					newVel, rotSpeed, pMarine );
			
			projectile->SetFreezeAmount(asw_fire_extinguisher_freeze_amount.GetFloat());
...
#endif
		// decrement ammo
		if (!asw_fire_extinguisher_infinite.GetBool())
			m_iClip1 -= 1;
