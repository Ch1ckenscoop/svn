...
{
	return asw_shotgun_piercing.GetInt();
}

float CASW_Weapon_Shotgun::GetPierceChance( void )
{
	return asw_shotgun_piercing_chance.GetFloat();
}
	...
	if (pMarine)		// firing from a marine
	{
		...
		if (true)		// hitscan pellets
		{
			...
			for (int i=0;i<iPellets;i++)
			{
				...
				pMarine->FirePenetratingBullets(info, GetPierceNum(), 1.0f, i, false );	//Ch1ckensCoop: Shotgun piercing
			}
		}
