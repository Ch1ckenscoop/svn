...
ConVar asw_tesla_gun_infinite("asw_tesla_gun_infinite", "0", FCVAR_CHEAT, "Infinite ammo for tesla gun.");

//static const float ASW_TG_RANGE = 240.0f; // Range of electrical discharge - TODO: make this an attribute?
ConVar ASW_TG_RANGE("asw_tesla_range", "240.0f", FCVAR_CHEAT, "Sets the range of the tesla gun.");
...
ConVar ASW_TG_SEARCH_DIAMETER("asw_tesla_search_diameter", "48.0f", FCVAR_CHEAT, "How far past the range to search for enemies to latch on to.");
...
ConVar ASW_TG_MAX_SHOCK_ALIENS("asw_tesla_max_targets", "6", FCVAR_CHEAT, "Max number of aliens to lock on to.", true, 0.0f, true, 32.0f);
...
{
	//Ch1ckenscoop: Act like chainsaw
	return true;
	//return m_iClip1 > 0;
}
...
void CASW_Weapon_Tesla_Gun::PrimaryAttack( void )
{
	...
	switch ( m_FireState )
	{
		...
		case ASW_TG_FIRE_DISCHARGE:
		{
			...
			if ( !hShockEntity )
			{
				//Ch1ckenscoop: Cap at a maximum of 32 shock targets
				//CBaseEntity *pList[ASW_TG_MAX_SHOCK_ALIENS.GetInt()];
			...
			{
				//Ch1ckenscoop: No ammo usage
				if (!asw_tesla_gun_infinite.GetBool())
