...
ConVar asw_flamer_freeze_infinite("asw_flamer_extinguisher_infinite", "0", FCVAR_CHEAT, "Infinite ammo for flamer secondary fire.");
ConVar asw_flamer_freeze_amount("asw_flamer_freeze_amount", "0.4", FCVAR_CHEAT, "Adjusts the freeze amount for the flamer secondary fire.");
...
void CASW_Weapon_Flamer::SecondaryAttack( void )
{
	...
	if (pMarine)		// firing from a marine
	{
		...
		if ( pMarine->GetWaterLevel() != 3 )
		{
			...
			projectile->SetFreezeAmount(asw_flamer_freeze_amount.GetFloat());
...
#endif
		// decrement ammo
		if (!asw_flamer_freeze_infinite.GetBool())
			m_iClip1 -= 1;
