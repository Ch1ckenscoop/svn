...
//================================================================//
//
// npc_BaseZombie.cpp has been ported from Valve source-sdk-2013.
//
//================================================================//
...
#include "asw_physics_prop_statue.h"	//stunning
...
//extern ConVar sk_npc_head;	//softcopy:
...
#define ZOMBIE_TORSO_HEALTH_FACTOR	0.7	//softcopy: was 0.5
...
//#define ZOMBIE_SWAT_DELAY			5	//softcopy: cvar
...
//ConVar	sk_zombie_dmg_both_slash( "sk_zombie_dmg_both_slash","0");	//not referenced
ConVar	sk_zombie_dmg_one_slash( "sk_zombie_dmg_one_slash","25", FCVAR_CHEAT, "Sets zombie slash damage");
ConVar	asw_zombie_swat_delay( "asw_zombie_swat_delay","5", FCVAR_CHEAT, "Sets zombie each swat delay");
...
bool CNPC_BaseZombie::FindNearestPhysicsObject( int iMaxMass )
{
	...
	for( i = 0 ; i < count ; i++ )
	{
		...
		if (!pCollide)	//softcopy: fix physcollision->CollideGetAABB() crashes
			continue;
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	//softcopy: make zombie immune to boomer blob/mortarbug shell
	if ( dynamic_cast<CASW_Boomer_Blob*>(info.GetAttacker()) || dynamic_cast<CASW_Mortarbug*>(info.GetAttacker()))
		return 0;
	// make the alien move slower for 0.5 seconds
	if (info.GetDamageType() & DMG_SHOCK)	//Ch1ckensCoop:
	{
...
bool CNPC_BaseZombie::ShouldIgnite( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_BURN )
	{
		...
		m_flBurnDamageResetTime = gpGlobals->curtime + 2.0 /*5*/;	//softcopy: less burn time
...
CBaseEntity *CNPC_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	//softcopy:
	if (ASWGameRules())
		iDamage = ASWGameRules()->ModifyAlienDamageBySkillLevel( iDamage );
...
{
	int nEvent = pEvent->Event();	//softcopy:

	if ( /*pEvent->event*/nEvent == AE_NPC_ATTACK_BROADCAST )
	{
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_POUND )
	{
		PoundSound();
		return;
	}

	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_ALERTSOUND )
	{
		AlertSound();
		return;
	}

	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_STEP_LEFT )
	{
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_STEP_RIGHT )
	{
		MakeAIFootstepSound( 180.0f );
		FootstepSound( true );
		return;
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_GET_UP )
	{
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_SCUFF_LEFT )
	{
		MakeAIFootstepSound( 180.0f );
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_SCUFF_RIGHT )
	{
		MakeAIFootstepSound( 180.0f );
		FootscuffSound( true );
		return;
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_STARTSWAT )
	{
		MakeAIFootstepSound( 180.0f );
		AttackSound();
		return;
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_ATTACK_SCREAM )
	{
		AttackSound();
		return;
	}

	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_SWATITEM )
	{
		...
		if ( pEnemy )
		{
			...
			//EmitSound( "NPC_BaseZombie.Swat" );	//softcopy:
			...
			m_flNextSwatScan = gpGlobals->curtime + asw_zombie_swat_delay.GetFloat();

			return;
		}
	}
	
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_ATTACK_RIGHT )
	{
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_ATTACK_LEFT )
	{
		Vector right, forward;
		AngleVectors( GetLocalAngles(), &forward, &right, NULL );
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_ATTACK_BOTH )
	{
		Vector forward;
	...
	if ( /*pEvent->event*/nEvent == AE_ZOMBIE_POPHEADCRAB )
	{
...
void CNPC_BaseZombie::Spawn( void )
{
	...
	GetEnemies()->SetFreeKnowledgeDuration( 0.5 );	//softcopy: was 6.0
...
void CNPC_BaseZombie::Precache( void )
{
	...
	PrecacheScriptSound( "ASW_Tesla_Laser.Damage" );	//softcopy:

	PrecacheModel( GetLegsModel() );
	PrecacheModel( GetTorsoModel() );

	PrecacheParticleSystem( "blood_impact_zombie_01" );
	PrecacheParticleSystem( "freeze_statue_shatter" );	//softcopy:
...
void CNPC_BaseZombie::StartTouch( CBaseEntity *pOther )
{
	...
	if (pOther && pOther->Classify() == CLASS_ASW_MARINE )	//softcopy: touch damage
	{
		int iZombieTouchDamage = 5;
		CTakeDamageInfo info( this, this, iZombieTouchDamage, DMG_ACID );
...
//softcopy:
void CNPC_BaseZombie::ElectroStun( float flStunTime )
{
	//Ch1ckensCoop:
...
void CNPC_BaseZombie::NPCThink( void )
{
	BaseClass::NPCThink();
	...
	if ( m_bElectroStunned && m_lifeState != LIFE_DYING )
	{
		...
		if ( m_fHurtSlowMoveTime < gpGlobals->curtime )	//Ch1ckensCoop:
		{
...
bool CNPC_BaseZombie::ModifyAutoMovement( Vector &vecNewPos )
{
...
bool CNPC_BaseZombie::ShouldMoveSlow() const
{
...
int	CNPC_BaseZombie::DrawDebugTextOverlays()
{
...
void CNPC_BaseZombie::Event_Killed( const CTakeDamageInfo &info )
{
	...
	//add blood effects and remove dead body 
	AddEffects( EF_NODRAW );
	AddFlag( FL_NOTARGET );
	m_spawnflags |= SF_NPC_GAG;
	trace_t tr;
	UTIL_TraceLine( GetAbsOrigin() + Vector( 0, 0, 64 ), GetAbsOrigin(), MASK_SOLID, this, COLLISION_GROUP_NONE, &tr );
...
bool CNPC_BaseZombie::CorpseGib( const CTakeDamageInfo &info )	//softcopy: add blood effect
{
	CEffectData	data;
...
/*bool CNPC_BaseZombie::CorpseGibBig( const CTakeDamageInfo &info )	//softcopy: add blood effect
{
	// Use the bone position to handle being moved by an animation (like a dynamic scripted sequence)
	static int s_nBodyBone = LookupBone( "Antlion.Body_Bone" );
	Vector vecOrigin;
	QAngle angBone;
