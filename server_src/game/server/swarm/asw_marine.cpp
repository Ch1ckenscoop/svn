...
extern ConVar asw_marine_electrified_armor_scale;	//Ch1ckensCoop: allow modifying the damage reduction amount of electrified armor.
		...
		if ( newInfo.GetDamageType() & DMG_SLASH )
		{
			if ( IsElectrifiedArmorActive() )
			{
				...
				if ( pAlien )
				{
					...
					newInfo.ScaleDamage( asw_marine_electrified_armor_scale.GetFloat() );
