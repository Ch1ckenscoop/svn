...
//Ch1ckensCoop: Removed these cvars and replaced them with defines since this class/entity isn't used in the game (too laggy/hitscan works better)
...
#define asw_shotgun_pellet_fade_length 100.0
#define asw_shotgun_pellets_pass false
...
void CASW_Shotgun_Pellet::CreateEffects( void )
{
	...
	if ( m_pMainGlow != NULL )
	{
		...
		m_pMainGlow->SetScale( 1.0f );	//Def 0.2
		m_pMainGlow->SetGlowProxySize( 4.0f );
		m_pMainGlow->SetColor(255, 64, 64);
	}
	...
	if ( m_pGlowTrail != NULL )
	{
		...
		m_pGlowTrail->SetMinFadeLength( asw_shotgun_pellet_fade_length );
		m_pGlowTrail->SetColor(255, 64, 64);
	}
}
...
void CASW_Shotgun_Pellet::PelletHit( CBaseEntity *pOther)
{
	...
	if ( pOther->m_takedamage != DAMAGE_NO )
	{
		...
		if (pSpawnable && asw_shotgun_pellets_pass)
		{
			m_hLastHit = pOther;
			EmitSound( "Weapon_Crossbow.BoltHitBody" );		//Ch1ckensCoop: Play "thwack" sound more than once.
			return;
		}
